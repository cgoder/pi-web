// PowerI 受控 fork of components/MessageView.tsx — 上游为准，重放增量。仅 import 差异。
"use client";
...
import { TurnWrittenFiles } from "@/components/TurnWrittenFiles";
...
function BlockView({ block, searchTarget, toolResults, isStreaming, streamingDuration, toolCallDurations, cwd, onOpenFile, onOpenSession, sessionId, entryId, blockIndex, writtenFiles }: { block: AssistantContentBlock; searchTarget?: boolean; toolResults?: Map<string, ToolResultMessage>; isStreaming?: boolean; streamingDuration?: number; toolCallDurations?: Map<string, number>; cwd?: string; onOpenFile?: (filePath: string) => void; onOpenSession?: (sessionId: string) => void; sessionId?: string; entryId?: string; blockIndex: number; writtenFiles?: WrittenFile[] }) {
  if (block.type === "text") {
    return <div data-message-text data-search-target={searchTarget || undefined}><TextBlock block={block as TextContent} isStreaming={isStreaming} cwd={cwd} onOpenFile={onOpenFile} writtenFiles={writtenFiles} /></div>;
  }
...
function TextBlock({ block, isStreaming, cwd, onOpenFile, writtenFiles }: { block: TextContent; isStreaming?: boolean; cwd?: string; onOpenFile?: (filePath: string) => void; writtenFiles?: WrittenFile[] }) {
  return <SafeMarkdownBody isStreaming={isStreaming} cwd={cwd} onOpenFile={onOpenFile} writtenFiles={writtenFiles}>{block.text}</SafeMarkdownBody>;
}
