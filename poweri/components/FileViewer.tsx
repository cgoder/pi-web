...
function DownloadLink({ filePath, sourceSessionId }: { filePath: string; sourceSessionId?: string | null }) {
  ...
  const handleDownload = async (e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    try {
      await downloadFile(filePath, sourceSessionId);
    } catch (err) {
    ...
    <div className="file-viewer-shell" style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      >
        <span
          ...
          style={{ fontFamily: "var(--font-mono)", cursor: "context-menu" }}
          title={filePath}
          onContextMenu={(e) => {
            e.preventDefault();
            // Store menu position in a global-ish way via custom event or state
            // For simplicity, dispatch a custom event that AppShell can handle
            const event = new CustomEvent("poweri:file-context-menu", {
              detail: { filePath, x: e.clientX, y: e.clientY },
            });
            ...
            <ReactMarkdown
              ...
              components={{
                ...
                a({ href, children, ...props }) {
                  ...
                  const apiHref = `/api/files/${encodeFilePathForApi(linkedFile)}?type=read${sourceSessionId ? `&sessionId=${encodeURIComponent(sourceSessionId)}` : ""}`;
                  return <a href={apiHref} {...props} onClick={handleClick}>{children}</a>;
                },
                img({ src, alt, ...props }) {
                  delete props.node;
