...
> 本节是产品负责人（用户）反复表达并亲自拍板的设计哲学，**任何功能/修复动手前先对照本节**：
> **分析方案 → 设计方案 → 实现方案**，三思后行。违反本节原则的改动会被要求返工。
> 来源：v0.2 活动栏 / 统计面板 / 历史会话时间线的迭代决策记录（issues 07/11/24/26）。

### 一、以人为本（Human-Centered）——第一性原则

产品的一切呈现首先服务**人**，其次才是数据或功能：

- **时间线是主框架，项目是次维度**。历史记录反映一个人的工作路径（"面向人"），
  项目只是路径上的标记。因此：天内严格**时间升序**、不按项目分组（用户会并行交替工作：
  等 A 项目模型响应时切到 B 项目）；项目身份用 chip 跟在时间后，绝不喧宾夺主。
- **位置稳定，阅读不打断**。点击展开详情时，被点击的行本身**纹丝不动**，详情在行下方
  全宽展开（宽度 = 行宽，横向空间最大化，高度自然更矮）。用户正在看的东西永远不跳走。
- **减少竖向压力**。小的可视化区块（圆环等）优先**并排横放**而非上下堆叠；
  天头/组头聚合摘要（会话数/tokens/费用/命中率）把重复信息压缩成一行。
- **可视化优先于数字罗列**。展示趋势、占比、量级用图（donut/heatmap/mini-bar/趋势条），
  降低阅读密集数字的心理负担；数字是图的下钻层，不是默认呈现。

### 二、原型先行（Prototype-First）——UI 必须看到效果

"UI you have to see"。任何 UI 功能**先出原型对比，用户拍板后再正式实现**：

- 默认提供 **2-3 个结构不同的变体**（如活动栏面板 / topbar 抽屉 / 独立整页），
  用 `?variant=` 切换 + 浮动切换条对比；变体必须结构性地不同，不是换皮。
- 原型阶段先验证**数据源与交互语义**（数据哪来、点击发生什么、布局怎么变），
  发现的设计问题（如重复 tab、窄布局拥挤）如实汇报，让用户决策。
- 用户选定后：**设计决策先问清楚再动手**（问清入口、形态、组合方式）；
  胜出设计 capture 到 throwaway 分支（`prototype/<feature>`）留档，
  正式实现只进主干，并更新 issue 记录决策与分支指针。
- 交互类小改（如行展开形态）可以直改原型验证，但**不经用户确认不得落入正式实现**。

### 三、上下文感知（Context-Aware）

面板/入口的行为必须与用户当前所处状态联动：

- 在会话内打开统计 → **当前会话优先**（tab 置首、默认展示）；无会话时只给历史+全局。
- 活动栏互斥语义（VS Code 式）：点已激活图标=收起；切图标=互斥切换。
- 入口能承载未来更多功能（活动栏面板可扩展为整页），设计时留扩展位。

### 四、双模式与默认全量

- 同类信息提供**可切换的两种视角**覆盖不同场景（如 按天↔按工作区：
  "找某天的事" vs "找某项目的事"；全局统计↔历史会话）。默认视角 = 用户最常需要的。
- **默认显示全部**，不做筛选下拉（数据量可承受时）；点击行才下钻细节。

### 五、自适应布局（Container Queries）

- 布局跟随**容器宽度**而非视口（同一组件既要在窄侧栏面板、也要在未来全屏页工作）：
  用 CSS `container-type: inline-size` + `@container` 断点，不用 `@media`。
- 断点设计：窄=上下堆叠 / 中=两列 / 宽=多列；统计卡片窄面板也要 2 列（不独占整行）。

### 六、数据真实与可对账

- 统计口径**与官方/SDK 一致**（如全文件累计含压缩历史，与 `get_session_stats` 同口径）；
  面板要标注口径（"全文件累计"），不与"当前上下文窗口"混淆。
- 重要数字**实测验证**（如安装体积、prune 收益做真机 e2e），不报估算值；
  对不上的账必须查清根因（如官方 override 缓存显示旧快照 = 上游缺陷，记 issue 不本地改）。
- 性能问题**单独立项研究**（量化 → 方案对比 → 决策），不阻塞当前功能。

### 七、视觉与细节规范

- 数字列**右对齐 + 固定列宽**，形成整齐的右边缘（标签左对齐可变长）。
- 时间用等宽字体（tabular-nums）、固定宽度（如 74px）右对齐，Git log 式。
- 颜色语义：费用/次要信息用弱化色，命中率等正面指标用绿色。
- 天/组头聚合元信息：`N 个会话 · X tokens · $Y · 缓存命中 Z%`（无数据项不显示）。
- 小圆环并排的间距/尺寸保持一致，中心显示总计。
