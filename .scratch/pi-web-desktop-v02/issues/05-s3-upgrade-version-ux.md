...
3. **版本显示**：设置区「关于」块只展示**壳版本**（tauri.conf.json productVersion = PowerI 版本，web 包即 PowerI 本体，不再单独显示 pi-web 版本）；升级按钮对托管副本（`cached`）与系统安装（`system`）均可用，点击后 `npm install --prefix <安装目录> @poweri/poweri-web@latest` 并重启
