...
## Настольное приложение (PowerI)

Ветка `desktop` добавляет **PowerI** — лёгкую нативную оболочку на базе [Tauri 2](https://tauri.app). Оболочка **не упаковывает** веб-приложение: она устанавливает пакет `@poweri/poweri-web` (закреплённый за версией оболочки) и запускает его как дочерний процесс, дожидается готовности настроенного порта и встраивает интерфейс в системный WebView (WKWebView на macOS, WebView2 на Windows) через iframe. Поскольку системный WebView заменяет собой встроенный Chromium, установочные пакеты весят около 2 МБ.

При первом запуске PowerI показывает **мастер настройки**, который проверяет окружение: наличие Node.js, наличие установленного `pi-web` в системе (включая корни fnm), и только если системная копия не найдена, переходит к установке `@poweri/poweri-web` через npm. Если Node.js отсутствует или версия слишком старая (< 22.19), мастер объясняет, что нужно исправить, прежде чем повторить попытку.

Требования: Rust toolchain для сборки; **Node.js 22.19+ во время выполнения** (приложение само запускает `npx`, глобальная установка не нужна).

```bash
npm install
npm run tauri dev      # режим разработки: next dev + оболочка vite, горячая перезагрузка
...
- В оболочке обязательно нужно передавать `--no-open`, иначе pi-web будет каждый раз открывать вкладку браузера из окна приложения.
