...
Ветка `desktop` добавляет **PowerI** — лёгкую нативную оболочку на базе [Tauri 2](https://tauri.app). Оболочка **не упаковывает** веб-приложение: она запускает `npx @agegr/pi-web --no-open` как дочерний процесс, дожидается готовности порта 30141 и встраивает интерфейс в системный WebView (WKWebView на macOS, WebView2 на Windows) через iframe. Поскольку системный WebView заменяет собой встроенный Chromium, установочные пакеты весят около 2 МБ.

При первом запуске PowerI показывает **мастер настройки**, который проверяет окружение: наличие Node.js, наличие установленного `pi-web` в системе (включая корни fnm), и только если системная копия не найдена, переходит к скачиванию через `npx`. Если Node.js отсутствует или версия слишком старая (< 22.19), мастер объясняет, что нужно исправить, прежде чем повторить попытку.
