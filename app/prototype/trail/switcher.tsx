...
// PROTOTYPE — variant switcher for the F2 trail prototype (mirror of
// the F1 switcher; variants differ so it can't be shared directly).
...
export function PrototypeSwitcher({ current }: { current: string }) {
  ...
  const setVariant = useCallback((key: string) => {
    ...
    router.replace(`/prototype/trail?${next.toString()}`);
  }, [params, router]);

  const cycle = useCallback((dir: 1 | -1) => {
    const idx = TRAIL_VARIANTS.findIndex((v) => v.key === current);
    const next = (idx + dir + TRAIL_VARIANTS.length) % TRAIL_VARIANTS.length;
    setVariant(TRAIL_VARIANTS[next].key);
  }, [current, setVariant]);
  ...
  const label = TRAIL_VARIANTS.find((v) => v.key === current) ?? TRAIL_VARIANTS[0];
  ...
  return (
    <div style={{
      ...
      <span style={{ opacity: 0.5, marginLeft: 6 }}>F2 轨迹原型 · mock 数据</span>
    </div>
  );
}
