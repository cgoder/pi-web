...
│   │                           #   安装 spec 包@CARGO_PKG_VERSION，托管目录 ~/.poweri/web）
│   └── logger.rs               # 滚动日志 → ~/.poweri/poweri.log
...
安装包产出 `src-tauri/target/release/bundle/`（`.dmg` / `-setup.exe` / `.msi`）。CI：`.github/workflows/build-poweri-desktop.yml`（触达 `src-tauri/**` 的推送构建矩阵）、`test-poweri-desktop.yml`（测试）；`poweri-v*` tag 触发发布——tag 一经推送不可移动，流程见 [`docs/desktop/release.md`](../docs/desktop/release.md)。

**版本三角锁**：`src-tauri/Cargo.toml` == `tauri.conf.json` == 根 `package.json` 三处版本必须一致（壳按自身版本号去 npm 拉 `@poweri/poweri-web@<版本>`）。
